<template>
  <div class="container">
    <div class="item item1">
      <QRbutton
        @click="changeButtonStatus('Text')"
        :class="ActiveButton === 'Text' ? 'buttonActive' : ''"
        buttonName="Text"
        Icon="fa-solid fa-file-lines"
      />
      <QRbutton
        @click="changeButtonStatus('Url')"
        :class="ActiveButton === 'Url' ? 'buttonActive' : ''"
        buttonName="Url"
        Icon="fa-solid fa-globe"
      />
      <QRbutton
        @click="changeButtonStatus('WiFi')"
        :class="ActiveButton === 'WiFi' ? 'buttonActive' : ''"
        buttonName="Wi-Fi"
        Icon="fa-solid fa-wifi"
      />
      <QRbutton
        disabled="true"
        @click="changeButtonStatus('Email')"
        :class="ActiveButton === 'Email' ? 'buttonActive' : ''"
        buttonName="Email"
        Icon="fa-solid fa-envelope"
      />
      <QRbutton
        disabled="true"
        @click="changeButtonStatus('Pdf')"
        :class="ActiveButton === 'Pdf' ? 'buttonActive' : ''"
        buttonName="Pdf"
        Icon="fa-solid fa-file-pdf"
      />
      <QRbutton
        disabled="true"
        @click="changeButtonStatus('Sms')"
        :class="ActiveButton === 'Sms' ? 'buttonActive' : ''"
        buttonName="SMS"
        Icon="fa-solid fa-sms"
      />
    </div>
    <div class="item item2">
      <QRDisplayerComponent
        :qrUrl="OriginalQrCodeImage_File"
        :key="OriginalQrCodeImage_File"
      />
      <a id="downloadButton" :href="OriginalQrCodeImage_File" download="qr.png"
        ><font-awesome-icon
...
async function callApi(apiEndPoint, Method, Data) {
  try {
    ...
    if (!response.ok) {
      ...
      OriginalQrCodeImage_File.value = objectURL;
    }
  } catch (error) {
  ...
  if (InputedData.value instanceof Object) {
    ...
    switch (InputedData.value.type) {
      case "wifi":
        ...
        callApi(
          "https://qr-code-generator-ebon-tau.vercel.app/generate-QrCode",
          "POST",
          data
        );
        break;
